...
/**
 * Approach 1: BFS 求转换过去的最少步数问题，第一反应为采用 BFS 来进行解决。
 *  以 "0000" 作
 * 起始位置开始进行遍历，然后寻找到 target 的最短路径即可。
 *  每个节点的邻居为每个位置上的数 +1, -1.
 *  这里为了保证范围落在 0~9 以内使用了 (x + MOD) % MOD => (x + 10) % 10 的方法。
 * 
 * 算是非常常用的一个小技巧，希望大家能够掌握，其次就是涉及到了 字符 与 整数 之间的转换。
 *  接下来的就是使用 BFS 的通用模板解决即可。（这里使用
 * 是出队列时判断的方法）
 */
class Solution {
            ...
            for (int i = 0; i < size; i++) {
                ...
                for (int pos = 0; pos < 4; pos++) {
                    for (int j = -1; j <= 1; j += 2) {
                        ...
                        chars[pos] = (char) (((chars[pos] - '0') + j + 10) % 10 + '0');
...
/**
 * Approach 2: BFS (Optimized) 首先，使用了 进队列 时进行判断的 BFS 模板，从而使得运行效率得到了一定的提高
 * 
...
/**
 * Approach 3: Bidirectional BFS 在 LeetCode 上发现还有更快的解法：
